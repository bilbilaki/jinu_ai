...
class SettingsService with ChangeNotifier {
  ...
  String _custoombaseurl = ''; // NOTE: Typo 'custoombaseurl' kept from original code
  String _setdefaultvoice = 'shimmer';
  String _defaultchatmodel = 'gpt-4.1-nano'; // Default from original
  ...
  String get custoombaseurl => _custoombaseurl; // NOTE: Typo kept
  ...
  @Deprecated('Use availableModels getter for IDs or getModelDetails for full data') Map<String, dynamic> get getmodels => _getmodels; // Keep for compatibility? Or remove.
  ...
  Future<void> loadSettings() async {
    ...
    _messageBufferSize = _prefs!.getInt(geminiMessageBufferSizeKey) ?? _messageBufferSize;
    ...
    _custoombaseurl = _prefs!.getString('custoombaseurl') ?? ''; // Key matches var name
    ...
    _setapisdkbaseurl = _prefs!.getString(settingsCustomBaseUrlKey) ?? ''; // Use const for this specific key
    _usagemode = _prefs!.getString('usagemode') ?? 'normal';
    _turnofftools = _prefs!.getBool('turnofftools') ?? false; // Use string key for bool

    // Load models fetched previously (both raw data and just IDs)
    _getmodels = (_prefs!.getString('modelsData') != null)
        ? json.decode(_prefs!.getString('modelsData')!) as Map<String, dynamic>
        : {};
    _availableModels = (_prefs!.getString('availableModels') != null)
        ? List<String>.from(json.decode(_prefs!.getString('availableModels')!))
        : [];


    _textprocessingmodel = _prefs!.getString('textprocessingmodel') ?? 'gpt-4.1-nano';
    _voiceprocessingmodel = _prefs!.getString('voiceprocessingmodel') ?? 'tts-1';
    _setdefaultvoice = _prefs!.getString('setdefaultvoice') ?? 'shimmer';
    _visionprocessingmodel = _prefs!.getString('visionprocessingmodel') ?? 'gpt-4.1-nano';
    _autotitle = _prefs!.getBool('autotitle') ?? true; // Use string key for bool
    _autotitlemodel = _prefs!.getString('autotitlemodel') ?? '';// Add default?
    _historyformodelsenabled = _prefs!.getBool('historyformodelsenabled') ?? true; // Use string key for bool
    _historybufferlength = _prefs!.getInt('historybufferlength') ?? 5; // Use string key for int
    _historychatenabled = _prefs!.getBool('historychatenabled') ?? true;// Use string key for bool
    _customsearchlocation = _prefs!.getString('customsearchlocation') ?? 'GB';
    _imagegenerationmodel = _prefs!.getString('imagegenerationmodel') ?? 'dall-e-3';
    _imagegenerationquality = _prefs!.getString('imagegenerationquality') ?? 'standard';
  ...
  Future<void> getModels() async {
    if (custoombaseurl.isEmpty || apitokenmain.isEmpty) {
      debugPrint("Cannot fetch models: Custom Base URL or Main API Token is missing.");
      _availableModels = [];
    ...
    String fetchUrl = custoombaseurl.endsWith('/models')
                      ? custoombaseurl
                      : (custoombaseurl.endsWith('/') ? '${custoombaseurl}models' : '$custoombaseurl/models');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        Map<String, dynamic> responseData = json.decode(utf8.decode(response.bodyBytes)); // Handle encoding
        ...
        _availableModels = modelsList
            .map<String?>((model) => model is Map ? model['id']?.toString() : null)
        ...
        await _prefs?.setString('modelsData', json.encode(_getmodels)); // Encode before saving
        ...
        debugPrint("Error fetching models: Status code ${response.statusCode}, Body: ${response.body}");
        _getmodels = {'error': 'Status code ${response.statusCode}', 'body': response.body};
        _availableModels = [];
        await _prefs?.remove('availableModels');
        await _prefs?.remove('modelsData'); // Optionally clear old data on error
      }
    ...
    if (_getmodels['data'] is List) {
      ...
      for (var model in models) {
        if (model is Map && model['id'] == modelId) {
          return Map<String, dynamic>.from(model); // Ensure it's the correct type
        }
      }
    }
    return null;
  ...
  Future<void> _setString(String key, String newValue, String currentValue) async {
      if (currentValue == newValue) return;
      // Update internal state immediately for responsiveness
      switch (key) {
          case 'custoombaseurl': _custoombaseurl = newValue; break;
          case 'defaultchatmodel': _defaultchatmodel = newValue; break;
          case 'apitokenmain': _apitokenmain = newValue; break;
          case 'apitokensub': _apitokensub = newValue; break;
          case 'setapisdkmodel': _setapisdkmodel = newValue; break;
          case settingsCustomBaseUrlKey: _setapisdkbaseurl = newValue; break; // Use const key
          case 'usagemode': _usagemode = newValue; break;
          case 'textprocessingmodel': _textprocessingmodel = newValue; break;
          case 'voiceprocessingmodel': _voiceprocessingmodel = newValue; break;
          case 'setdefaultvoice': _setdefaultvoice = newValue; break;
          case 'visionprocessingmodel': _visionprocessingmodel = newValue; break;
          case 'autotitlemodel': _autotitlemodel = newValue; break;
          case 'customsearchlocation': _customsearchlocation = newValue; break;
          case 'imagegenerationmodel': _imagegenerationmodel = newValue; break;
          case 'imagegenerationquality': _imagegenerationquality = newValue; break;
          ...
          case 'customapiurl': _customapiurl = newValue; break;
          case 'customapikey': _customapikey = newValue; break;
          case 'customapiparam': _customapiparam = newValue; break;
          case 'customapiparamvalue': _customapiparamvalue = newValue; break;
          case 'customapiheaders': _customapiheaders = newValue; break;
          case 'customapitype': _customapitype= newValue; break;
          case geminiSystemInstructionKey: _systemInstruction = newValue; break; // Use const key
          default: debugPrint("Warning: Unhandled key in _setString: $key"); return;
      }
  ...
  Future<void> _setBool(String key, bool newValue, bool currentValue) async {
      if (currentValue == newValue) return;
       switch (key) {
         case 'turnofftools': _turnofftools = newValue; break;
         case 'autotitle': _autotitle = newValue; break;
         case 'historyformodelsenabled': _historyformodelsenabled = newValue; break;
         case 'historychatenabled': _historychatenabled = newValue; break;
         case 'devmod': _devmod = newValue; break;
         default: debugPrint("Warning: Unhandled key in _setBool: $key"); return;
       }
      notifyListeners();
      await _prefs?.setBool(key, newValue); // Use string key for bools per original load logic
  }
  ...
  Future<void> _setInt(String key, int newValue, int currentValue) async {
      if (currentValue == newValue) return;
       switch (key) {
         ...
         default: debugPrint("Warning: Unhandled key in _setInt: $key"); return;
       }
      notifyListeners();
      // Use const keys for specific Gemini params
      if (key == geminiTopKKey || key == geminiMaxTokensKey || key == geminiMessageBufferSizeKey){
           await _prefs?.setInt(key, _getIntValue(key)); // Save clamped value
      } else {
          await _prefs?.setInt(key, _getIntValue(key)); // Save potentially unclamped value using string key
      }
    }
  ...
  Future<void> _setDouble(String key, double newValue, double currentValue) async {
      ...
      notifyListeners();
      // Use const keys for specific Gemini params
      await _prefs?.setDouble(key, _getDoubleValue(key)); // Save potentially clamped value
    }
  ...
  Future<void> setCustoombaseurl(String value) => _setString('custoombaseurl', value, _custoombaseurl);
  Future<void> setDefaultchatmodel(String value) => _setString('defaultchatmodel', value, _defaultchatmodel);
  Future<void> setApitokenmain(String value) => _setString('apitokenmain', value, _apitokenmain);
  Future<void> setApitokensub(String value) => _setString('apitokensub', value, _apitokensub);
  Future<void> setApisdkmodel(String value) => _setString('setapisdkmodel', value, _setapisdkmodel);
  Future<void> setApisdkbaseurl(String value) => _setString(settingsCustomBaseUrlKey, value, _setapisdkbaseurl); // Use Const Key
  Future<void> setUsagemode(String value) => _setString('usagemode', value, _usagemode);
  Future<void> setTurnofftools(bool value) => _setBool('turnofftools', value, _turnofftools);
  // setGetmodels is not needed, it's populated by getModels()
  Future<void> setTextprocessingmodel(String value) => _setString('textprocessingmodel', value, _textprocessingmodel);
  Future<void> setVoiceprocessingmodel(String value) => _setString('voiceprocessingmodel', value, _voiceprocessingmodel);
  Future<void> setDefaultvoice(String value) => _setString('setdefaultvoice', value, _setdefaultvoice);
  Future<void> setVisionprocessingmodel(String value) => _setString('visionprocessingmodel', value, _visionprocessingmodel);
  Future<void> setAutotitle(bool value) => _setBool('autotitle', value, _autotitle);
  Future<void> setAutotitlemodel(String value) => _setString('autotitlemodel', value, _autotitlemodel);
  Future<void> setHistoryformodelsenabled(bool value) => _setBool('historyformodelsenabled', value, _historyformodelsenabled);
  Future<void> setHistorybufferlength(int value) => _setInt('historybufferlength', value, _historybufferlength);
  Future<void> setHistorychatenabled(bool value) => _setBool('historychatenabled', value, _historychatenabled);
  Future<void> setCustomsearchlocation(String value) => _setString('customsearchlocation', value, _customsearchlocation);
  Future<void> setImagegenerationmodel(String value) => _setString('imagegenerationmodel', value, _imagegenerationmodel);
  Future<void> setImagegenerationquality(String value) => _setString('imagegenerationquality', value, _imagegenerationquality);
  ...
  Future<void> setCustomapiurl(String value) => _setString('customapiurl', value, _customapiurl);
  Future<void> setCustomapikey(String value) => _setString('customapikey', value, _customapikey);
  Future<void> setCustomapiparam(String value) => _setString('customapiparam', value, _customapiparam);
  Future<void> setCustomapiparamvalue(String value) => _setString('customapiparamvalue', value, _customapiparamvalue);
  Future<void> setCustomapiheaders(String value) => _setString('customapiheaders', value, _customapiheaders);
  Future<void> setCustomapitype(String value) => _setString('customapitype', value, _customapitype);
  ...
  Future<void> setMessageBufferSize(int value) => _setInt(geminiMessageBufferSizeKey, value, _messageBufferSize);// Use Const Key
  ...
  Future<void> setThemeMode(ThemeMode mode) async {
    ...
    await _prefs?.setString(appThemeModeKey, mode.toString()); // Save asynchronously
  }
  ...
  dynamic getValue(String key) {
    switch (key) {
    // Gemini Specific
      case 'temperature': case geminiTemperatureKey: return _temperature;
      case 'topK': case geminiTopKKey: return _topK;
      case 'topP': case geminiTopPKey: return _topP;
      case 'maxOutputTokens': case geminiMaxTokensKey: return _maxOutputTokens;
      case 'systemInstruction': case geminiSystemInstructionKey: return _systemInstruction;
      case 'messageBufferSize': case geminiMessageBufferSizeKey: return _messageBufferSize;
    // App Theme
      case 'themeMode': case appThemeModeKey: return _themeMode;
    // Other Settings (Using variable names mostly)
      case 'custoombaseurl': return _custoombaseurl;
      case 'setdefaultvoice': return _setdefaultvoice;
      case 'defaultchatmodel': return _defaultchatmodel;
      case 'apitokenmain': return _apitokenmain;
      case 'apitokensub': return _apitokensub;
      case 'setapisdkmodel': return _setapisdkmodel;
      case 'setapisdkbaseurl': case settingsCustomBaseUrlKey: return _setapisdkbaseurl; // Handles both
      case 'usagemode': return _usagemode;
      case 'turnofftools': return _turnofftools;
      case 'getmodels': return _getmodels;
      case 'availableModels': return availableModels;
      case 'textprocessingmodel': return _textprocessingmodel;
      case 'voiceprocessingmodel': return _voiceprocessingmodel;
      case 'visionprocessingmodel': return _visionprocessingmodel;
      case 'autotitle': return _autotitle;
      case 'autotitlemodel': return _autotitlemodel;
      case 'historyformodelsenabled': return _historyformodelsenabled;
      case 'historybufferlength': return _historybufferlength;
      case 'historychatenabled': return _historychatenabled;
      case 'customsearchlocation': return _customsearchlocation;
      case 'imagegenerationmodel': return _imagegenerationmodel;
      case 'imagegenerationquality': return _imagegenerationquality;
      case 'imagegenerationsize': return _imagegenerationsize;
      case 'imageanalysismodeldetails': return _imageanalysismodeldetails;
      case 'credits': return _credits;
      case 'customoutputstyle': return _customoutputstyle;
      case 'promptsbookmark': return _promptsbookmark;
      case 'customapiname': return _customapiname;
      case 'customapitoken': return _customapitoken;
      case 'customapiurl': return _customapiurl;
      case 'customapikey': return _customapikey;
      case 'customapiparam': return _customapiparam;
      case 'customapiparamvalue': return _customapiparamvalue;
      case 'customapiheaders': return _customapiheaders;
      case 'customapitype': return _customapitype;
      case 'devmod': return _devmod;
      default:
        debugPrint('SettingsService: Unknown setting key requested in getValue: $key');
        return null;
    }
  }
